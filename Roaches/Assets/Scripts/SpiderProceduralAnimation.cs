...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        defaultLegPos = new Vector3[nbLegs];
        lastLegPos = new Vector3[nbLegs];
        legMoving = new bool[nbLegs];
        for (int i = 0; i < nbLegs; ++i)
        {
            defaultLegPos[i] = legTargets[i].localPosition;
    ...
    {
        Vector3 startPos = lastLegPos[index];
        ...
        lastLegPos[index] = legTargets[index].position;
        legMoving[0] = false;
    }
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < nbLegs; ++i)
        {
            desiredPositions[i] = transform.TransformPoint(defaultLegPos[i]);

            float distance = Vector3.ProjectOnPlane(desiredPositions[i] + velocity * velocityMultiplier - lastLegPos[i], transform.up).magnitude;
        ...
        if (indexToMove != -1 && !legMoving[0])
        {
            ...
            Vector3[] positionAndNormalFwd = MatchToSurfaceFromAbove(targetPoint + velocity * velocityMultiplier, Range, (transform.parent.up - velocity * 100).normalized);
            Vector3[] positionAndNormalBwd = MatchToSurfaceFromAbove(targetPoint + velocity * velocityMultiplier, Range*(1f + velocity.magnitude), (transform.parent.up + velocity * 75).normalized);
